...
@interface ZooNetworkInterceptor()

@property (nonatomic, strong) NSHashTable *delegates;
@end

@implementation ZooNetworkInterceptor
...
+ (instancetype)shareInstance {
    ...
    dispatch_once(&once, ^{
        instance = [[ZooNetworkInterceptor alloc] init];
    });
    return instance;
}
...
- (BOOL)shouldIntercept {
    ...
    for (id<ZooNetworkInterceptorDelegate> delegate in self.delegates) {
...
- (void)updateURLProtocolInterceptStatus {
    if (self.shouldIntercept) {
        [NSURLProtocol registerClass:[ZooNSURLProtocol class]];
    }else{
        [NSURLProtocol unregisterClass:[ZooNSURLProtocol class]];
    }
}
...
- (void)handleResultWithData: (NSData *)data
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        for (id<ZooNetworkInterceptorDelegate> delegate in self.delegates) {
            [delegate zooNetworkInterceptorDidReceiveData:data response:response request:request error:error startTime:startTime];
        }
    });
}
