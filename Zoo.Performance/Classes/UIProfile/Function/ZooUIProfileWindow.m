...
@implementation ZooUIProfileWindow

+ (instancetype)sharedInstance
{
    static ZooUIProfileWindow *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [ZooUIProfileWindow new];
    });
...
- (void)pan:(UIPanGestureRecognizer *)sender{
    ...
    if (newX > ZooScreenWidth - kWindowWidth/2) {
        newX = ZooScreenWidth - kWindowWidth/2;
    }
    if (newY < self.zoo_height/2) {
        newY = self.zoo_height/2;
    }
    if (newY > ZooScreenHeight - self.zoo_height/2) {
        newY = ZooScreenHeight - self.zoo_height/2;
    }
