 ...
 *  卡顿回调
 */
@property (nonatomic, copy) ZooANRTrackerBlock handler;
    ...
    void (^ verifyReport)(void) = ^() {
        ...
        if (strongSelf.reportInfo.length > 0) {
            if (strongSelf.handler) {
                ...
                strongSelf.handler(@{
                                     @"title": [ZooUtil dateFormatNow].length > 0 ? [ZooUtil dateFormatNow] : @"",
    ...
    while (!self.cancelled) {
        if (_isApplicationInActive) {
            ...
            if (self.isMainThreadBlock) {
                self.reportInfo = [ZooBacktraceLogger zoo_backtraceOfMainThread];
            }
