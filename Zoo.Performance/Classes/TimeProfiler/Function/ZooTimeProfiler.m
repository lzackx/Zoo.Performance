...
@implementation ZooTimeProfiler
...
+ (NSString *)getRecordsResult {
    ...
    NSArray<ZooTimeProfilerRecord *>*arr = [self getRecords];
    for (ZooTimeProfilerRecord *r in arr) {
...
+ (void)appendRecord:(ZooTimeProfilerRecord *)record to:(NSMutableString *)str {
    [str appendFormat:@"%@\n", [record descriptionWithDepth]];
    for (ZooTimeProfilerRecord *r in record.subRecords) {
        [self appendRecord:r to:str];
    }
}

+ (NSArray<ZooTimeProfilerRecord *>*)getRecords {
    NSMutableArray<ZooTimeProfilerRecord *> *arr = [NSMutableArray new];
    int record_num = 0;
    dtp_call_record *records = dtp_get_call_records(&record_num);
    for (int i = 0; i < record_num; i++) {
        dtp_call_record *rec = &records[i];
        
        ZooTimeProfilerRecord *r = [ZooTimeProfilerRecord new];
