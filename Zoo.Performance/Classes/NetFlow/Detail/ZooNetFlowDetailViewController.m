//
//  ZooNetFlowDetailViewController.m
//  Zoo
//
...
@interface ZooNetFlowDetailViewController ()<UITableViewDelegate,UITableViewDataSource,ZooNetFlowDetailSegmentDelegate>

@property (nonatomic, strong) ZooNetFlowDetailSegment *segmentView;
...
@implementation ZooNetFlowDetailViewController
...
- (void)viewDidLoad {
    ...
    self.title = ZooLocalizedString(@"网络监控详情");
    
    _segmentView = [[ZooNetFlowDetailSegment alloc] initWithFrame:CGRectMake(0, IPHONE_NAVIGATIONBAR_HEIGHT, self.view.zoo_width, kZooSizeFrom750_Landscape(88))];
    _segmentView.delegate = self;
    [self.view addSubview:_segmentView];
    
    CGFloat tabBarHeight = self.tabBarController.tabBar.zoo_height;
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, _segmentView.zoo_bottom, self.view.zoo_width, self.view.zoo_height-tabBarHeight-_segmentView.zoo_bottom) style:UITableViewStyleGrouped];
...
- (void)initData{
    ...
    NSString *requestDataSize = [NSString stringWithFormat:ZooLocalizedString(@"数据大小 : %@"),[ZooUtil formatByte:[self.httpModel.uploadFlow floatValue]]];
    ...
    _requestArray = @[@{
                          @"sectionTitle":ZooLocalizedString(@"请求概要"),
                          @"dataArray":@[requestDataSize,method]
                          },
                      @{
                          @"sectionTitle":ZooLocalizedString(@"链接"),
                          @"dataArray":@[linkUrl]
                          },
                      @{
                          @"sectionTitle":ZooLocalizedString(@"请求头"),
                          @"dataArray":@[allHTTPHeaderString]
                          },
                      @{
                          @"sectionTitle":ZooLocalizedString(@"请求体"),
                          @"dataArray":@[requestBody]
                          }
                      ];
    
    NSString *respanseDataSize = [NSString stringWithFormat:ZooLocalizedString(@"数据大小 : %@"),[ZooUtil formatByte:[self.httpModel.downFlow floatValue]]];
    ...
    _responseArray = @[@{
                          @"sectionTitle":ZooLocalizedString(@"响应概要"),
                          @"dataArray":@[respanseDataSize,mineType]
                          },
                      @{
                          @"sectionTitle":ZooLocalizedString(@"响应头"),
                          @"dataArray":@[responseHeaderString]
                          },
                      @{
                          @"sectionTitle":ZooLocalizedString(@"响应体"),
...
#pragma mark - ZooNetFlowDetailSegmentDelegate
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [ZooNetFlowDetailCell cellHeightWithContent:content];
}

- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    return kZooSizeFrom750_Landscape(100);
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.zoo_width, kZooSizeFrom750_Landscape(100))];
    
    UILabel *tipLabel = [[UILabel alloc] init];
    tipLabel.textColor = [UIColor zoo_colorWithHex:0x337CC4];
    tipLabel.font = [UIFont systemFontOfSize:kZooSizeFrom750_Landscape(32)];
    tipLabel.text = title;
    tipLabel.frame = CGRectMake(kZooSizeFrom750_Landscape(32), 0, self.view.zoo_width-kZooSizeFrom750_Landscape(32), kZooSizeFrom750_Landscape(100));
    [view addSubview:tipLabel];
    //tipLabel.backgroundColor = [UIColor zoo_colorWithHex:0xeff0f4];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZooNetFlowDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:identifer];
    if (!cell) {
        cell = [[ZooNetFlowDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifer];
    }
